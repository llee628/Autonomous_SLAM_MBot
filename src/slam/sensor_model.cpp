...
double SensorModel::likelihood(const particle_t& sample, const lidar_t& scan, const OccupancyGrid& map)
{
    ...
    for(auto & ray: movingScan){
        Point<double> endpoint(ray.origin.x + ray.range * std::cos(ray.theta),
                               ray.origin.y + ray.range * std::sin(ray.theta));
        auto rayEnd = global_position_to_grid_position(endpoint, map);
        Point<double> end_before(ray.origin.x + (ray.range-0.05f) * std::cos(ray.theta),
                               ray.origin.y + (ray.range-0.05f) * std::sin(ray.theta));
        auto rayBefore = global_position_to_grid_position(end_before, map);

        if(map.logOdds(rayEnd.x, rayEnd.y) >= 0.0){
            if(map.logOdds(rayBefore.x, rayBefore.y) > 0.0){
                scanScore += 0.5*(map.logOdds(rayBefore.x, rayBefore.y) + map.logOdds(rayEnd.x, rayEnd.y));
            }
            else{
